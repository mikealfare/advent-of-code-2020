...
def test_update_with_the_next_number(numbers, expected):
    last_seen = day_15.get_last_seen_dict(numbers)
    day_15.update_with_the_next_number(numbers, last_seen)
    assert numbers[-1] == expected
    assert last_seen == day_15.get_last_seen_dict(numbers)
